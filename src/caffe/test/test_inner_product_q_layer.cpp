...
protected:
    InnerProductQLayerTest()
            : blob_bottom_(new Blob<Dtype>(vector<int>{8})),
              blob_top_(),
              D_(new Blob<Dtype>(1, 1, 8, 2)),
              bias_(new Blob<Dtype>(vector<int>{5})),
              B_(new Blob<Dtype>(vector<int>{1})){
        // fill the values
        for (int i = 0; i < 8; i++) {
        ...
        B_->mutable_cpu_data()[0] = Dtype(147779584);
        caffe_set(5, Dtype(0), bias_->mutable_cpu_data());
...
TYPED_TEST(InnerProductQLayerTest, TestForward) {
    ...
    if (Caffe::mode() == Caffe::CPU ||
        ...
        layer->blobs() = vector<shared_ptr<Blob<Dtype>>{D_, bias_, B_};
        layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
        layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
        const Dtype *data = this->blob_top_->cpu_data();
        const int count = this->blob_top_->count();
        const Dtype correct[] = {50, 66, 78, 86, 90};
